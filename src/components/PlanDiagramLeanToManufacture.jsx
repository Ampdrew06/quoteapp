...
// tighter drawing margins, but still enough room for labels
const M_TOP = 12;
const M_RIGHT = 20;
const M_BOTTOM = 54;
const M_LEFT = 10;
...
const scale = Math.min(
  innerW / Math.max(1, extW),
  innerH / Math.max(1, extP)
);

const ow = extW * scale;
const oh = extP * scale;

// centre the whole drawing block within the available area
const ox = M_LEFT + (innerW - ow) / 2;
const oy = M_TOP + (innerH - oh) / 2 + 14;
...
>
      <defs>
        <marker id={arrId} markerWidth="10" markerHeight="10" refX="5" refY="3" orient="auto-start-reverse">
          <path d="M0,0 L6,3 L0,6" fill="none" stroke="#374151" strokeWidth="1.5" />
        </marker>
        <marker id={arrYId} markerWidth="10" markerHeight="10" refX="5" refY="3" orient="auto-start-reverse">
          <path d="M0,0 L6,3 L0,6" fill="none" stroke="#374151" strokeWidth="1.5" />
        </marker>
      </defs>
      ...
      {/* --- dimensions --- */}
      <DimX x1={ix} x2={ix + iw_px} y={oy - 8} text={`${dim(iw)} int`} />
      <DimX x1={ox} x2={ox + ow} y={oy + oh + 36} text={`${dim(extW)} ext`} />
      <DimY x={ox - 28} y1={oy} y2={oy + oh} text={`${dim(extP)} ext`} />
<DimY x={ox + ow + 28} y1={oy + oh - ip_px - (soffit * scale)} y2={oy + oh - (soffit * scale)} text={`${dim(ip)} int`} />

{/* --- pitch at centre of drawing --- */}
<text
  x={ox + ow / 2}
  y={oy + oh / 2}
  textAnchor="middle"
  dominantBaseline="middle"
  style={{ font: "16px Inter, system-ui, Arial", fontWeight: 700 }}
  fill="#111827"
>
  {round(pitchDeg, 1)}°
</text>

{/* --- side labels on actual external edges, inside the roof outline --- */}
<g>
  <text
    x={ox + 14}
    y={oy + oh / 2}
    ...
    style={{ font: "12px Inter, system-ui, Arial", fontWeight: 700 }}
    fill="#111827"
  >
    {leftWall ? "WALL" : "END"}
  </text>
